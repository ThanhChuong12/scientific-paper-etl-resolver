...
\author{Rituparna Mandal} \email{drimit.ritu@gmail.com}
\affiliation{Department of Physics, National Central University, Zhongli, Taoyuan 320317, Taiwan}
...
due to the negative mass dimension of the gravitational constant. However, in recent years, the asymptotically safe
...
integrated out. More explicitly the FRG equation is
\bea
\frac{d\Gamma_k}{dt} = \frac12 {\rm Tr}\left[ \Gamma_k^{(2)} + R_k \right]^{-1} \frac{dR_k}{dt},
\label{frge}
\eea
where $\Gamma_k^{(2)}$ is the second variation of the effective average action and $t=\log(k/k_0)$ with $k_0$
being an arbitrary initial value.
...
\bea
S = \int d^4 x \sqrt{-g} \frac{1}{16 \pi G} (R - 2 \Lambda),
\eea
where $G$ is the Newton constant and $\Lambda$ is the cosmological constant.
...
equation~\cite{Reuter:1996cp, Bonanno:2001xi, Codello:2008vh}.
...
\label{eq_GLam}
\end{eqnarray}
where $G_0$ and $\Lambda_0$ are the Newton coupling and cosmological constant at $k=0$. The dimensionless parameters
...
the RG trajectories with a positive cosmological constant.
...
We note that the quantum correction to the cosmological constant begins at the $k^4$ order for $\Lambda_0 = 0$,
...
quantum corrections into the Einstein equations by replacing the ordinary gravitational constant and cosmological
constant with their scale-dependent counterparts. This approach allows us to explore the impact of quantum gravity
...
of differential equations involving the Hubble parameter, energy density, the scale-dependent Newton's gravitational
constant, the cosmological constant, and a cutoff function. An important input for defining these equations is
...
the time-dependent gravitational constant, and the cosmological constant. Many cosmological studies have employed
this modified continuity equation approach~\cite{Bonanno:2007wg, Reuter:2003ca, Reuter:2003ca, Reuter:2004nv, lit1,
...
For the $\Lambda_0 \neq 0$ branch,
% though it may appear that this might not be connected smoothly to the branch of $\Lambda_0 = 0$, it may be of
% some use to study what quantum effects appear.
...
In this section, the FRG improvement is applied at the ``equation level'' by replacing the ordinary Newtonian
gravitational constant and cosmological constant with the scale-dependent running coupling constants $G(k)$
and $\Lambda(k)$ in the Einstein equation. We take the viewpoint that these running coupling constants capture
...
the study of cosmology, where the infrared cutoff is identified as $k=k(t)$ with the cosmic time $t$.
...
When we consider the quantum effects, $G$ and $\Lambda$ depend on the cosmic time. This arises because of the quantum effects of gravity. It is then expected that they also contribute to the
...
= - \alpha \left( H^2 - \frac{\Lambda}3 \right)\,,
\label{eq_Ein_H}
\end{align}
where $\alpha \equiv 3 (1 + w)/2$.
Furthermore, the energy density $\rho(t)$ can be expressed in terms of the solution for $H(t)$ and
the cosmological constant $\Lambda(t)$:
\begin{eqnarray}
\rho = \frac3{8 \pi G} \left( H^2 - \frac{\Lambda}3 \right). \label{eq_Ein_rho}
\end{eqnarray}
...
the external function $\Lambda(t)$ is required. The running gravitational constant $G(t)$ only appears
...
or the curvature of spacetime~\cite{Bonanno:2001xi}. However, there is no satisfactory principles to directly
...
homogeneity and isotropy imply that the only physical sacle is the Hubble parameter which is determined
by the cosmic time. Thus we have
%
\begin{eqnarray}
...
$k(H) = \xi G_0^{(\beta - 1)/2} H^\beta$, where $\beta$ is constant.
...
Substituting the cosmological constant~\eqref{eq_GLam0_t} into~\eqref{eq_Ein_H}, and taking the ansatz for $H(t)$ as
%
\begin{equation}
H(t) = \frac{1}{\alpha t} \left( 1 + \frac{c}{t} + \frac{c_1}{t^2} + \frac{c_2}{t^3} + \frac{c_3}{t^4} + \cdots \right)~,
\label{eq_ansa}
\end{equation}
%
...
the cosmological constant term, in contrast to classical case. But in the absence of the quantum corrections
...
the optimized and exponential cutoffs: specifically, $\tilde\nu = \xi^4/8\pi$ for the former case,
...
The leading-order quantum correction to the cosmological constant, $\Lambda \simeq \tilde\nu G_0/t^4 > 0$,
...
\bea
k(H) = \zeta G_0^{(\beta - 1)/2} H^\beta,
\eea
where $\zeta$ is a dimensionless constant. To ensure that $\zeta$ in the cutoff scale is dimensionless, unlike in
...
we obtain the leading correction to the cosmological constant as
\begin{equation}
...
% For $\beta<1/2$, the infrared cutoff scale $k$ scales with Planck mass ($m_{Pl}=G_0^{-\frac{1}{2}}$),
%implying a large value for $k$  and, consequently, an unreasonably large value for $\Lambda$.
%Since the expansions of $G(k)$ and $\Lambda(k)$ are valid only at small momentum scales (i.e., at late times),
%the solutions for the Hubble parameter and the scale factor are consistent only for $\beta > 1/2$.
...
is exactly $\beta = 1/2$. This gives $k = \zeta G_{0}^{-1/4} H^{1/2} = \zeta \sqrt{m_\mathrm{Pl}} H^{1/2}$ and the leading order cosmological constant becomes $\Lambda = \zeta^4 \nu H^{2}$.
The Friedmann equation then takes the form
\begin{equation}
\frac{d H}{d t} = - \alpha \left( 1 - \frac{\zeta^4 \nu}{3} \right) H^2~.
\label{fridb12}
\end{equation}
Solving this equation, we find the Hubble parameter as a function of cosmic time:
\begin{equation}
H(t) = \frac{1}{\alpha (1 - \zeta^4 \nu/3) (t - c)},
\end{equation}
where $c$ is an arbitrary integration constant. Integrating $H(t)$, the scale factor can be written as
\begin{equation}
a(t) = a_0 (t - c)^{\frac{1}{\alpha (1 - \zeta^4 \nu/3)}}~.
\end{equation}
...
cosmological constant does not depend on the Planck mass and remains small.
...
for the Hubble parameter $H_{\mathrm{cl}}(t)$ in a universe dominated by a cosmological constant,
...
By inserting this cutoff scale in Eq.~\eqref{eq_GLam}, we can express the running gravitational coupling
and cosmological coupling as functions of time as
\begin{eqnarray}
...
However, the classical solution shows the exponential expansion, which can never reproduced from the power
expansion in $t$.
...
\end{equation}
where we have used the cosmological constant to introduce dimension, and we use this for identification
\bea
...
With the choice of cutoff~\eqref{cf}, the scale-dependent cosmological constant~\eqref{eq_GLam} becomes
\begin{equation}
...
%
%{\red Obviously, for $c' > 0$, the quantum corrections enhance the cosmic expansion eternally. However, for $c' < 0$, the Hubble parameter~\eqref{solHtau} can be reexpressed as
%\begin{equation}
%H(\tau) = \sqrt{\Lambda_0/3} \left[ 1 + \frac{2 |c'|}{\tau} \left( \frac{\tau_c}{\tau} - 1 \right) - \frac{2 |c'|^2}{\tau^2} \left( \frac{\tau_c}{\tau} - 1 \right) + \cdots \right]~,
%\end{equation}
%and the parameter $|c'|$ determined a critical scalae
%\begin{equation}
%\tau_c = \tilde\mu G_0 / 4 |c'|,
%\end{equation}
%such that for late time, $\tau > \tau_c$ the quantum effects turned into suppressing cosmic expansion.
%}
...
in asymptotically safe gravity has resulted in the running of the gravitational constant and the cosmological
constant under the Einstein-Hilbert truncation. We have analyzed the cosmic evolution of the FLRW cosmology
at late times by improving the Einstein equations to include the scale dependence of Newton's constant and
the cosmological constant. Specifically, we have considered the conservation of the entire right-hand side
...
The first branch has involved a power series expansion of the dimensionful cosmological constant, $\Lambda(t)$,
...
starts with a $k^2$ term in the expansion of the cosmological constant and is valid up to a certain
non-zero momentum scale, where the $\beta$-function becomes singular as the dimensionless cosmological
constant $\lambda(k)$ comes to a ``singular line'' near $1/2$.
...
$k(H) = \zeta G_0^{(\beta - 1)/2} H^\beta$, where $\beta$ is a constant. In earlier work~\cite{Mandal:2020umo},
...
On the other hand, for $\beta = 3/4$ and $\beta = 1$, entropy generation converges to a constant and thus
these viable cutoff choices are viable. This suggests the existence of a critical value of $\beta$ that
...
$k = \xi/t$ in terms of the cosmic time, but found that it does not provide a consistent solution because the quantum corrections to the Hubble parameter introduce power expansion terms that become more
...
\\
k \partial_k \Lambda(k) &=& \eta_N \Lambda + \frac{k^4 G(k)}{2 \pi}
...
\end{equation}
in which the dimensionless running couplings are defined as $g(k) = G(k) k^2, \lambda(k) = \Lambda(k)/k^2$ and
$B_1(\lambda), B_2(\lambda)$ are abbreviations of
\begin{equation}
...
\end{equation}
The threshold functions that appeared here are defined as
\begin{equation}
...
{\tilde\Phi}^p_n(w) = \frac1{\Gamma(n)} \int_0^\infty dz z^{n - 1} \frac{R^{(0)}(z)}{\left[ z + R^{(0)}(z)
 + w \right]^p},
\end{equation}
which depend on the choice the cutoff function $R^{(0)}(z)$.
...
\noindent{\bf Case I: $\Lambda_0 = \Lambda_2 = 0$ and $\Lambda_i = G_i = 0$ for odd $n$}
...
\end{eqnarray}
The solutions agree with the results in~\cite{Bonanno:2001xi} as, in the notation of~\eqref{eq_GLam}:
$G_2 = - \omega G_0^2, G_4 = \omega_1 G_0^3, \Lambda_4 = \nu G_0, \Lambda_6 = \nu_1 G_0^2$,
\begin{equation}
\label{eq_exp_0}
...
\end{eqnarray}
With the specific values~\eqref{eq_Phi_val} we have
\begin{eqnarray}
G(k) &=& G_0 \left[ 1 - \frac{9 + \pi^2}{9 \pi} G_0 k^2 + \left[ \left( \frac{9 + \pi^2}{9 \pi} \right)^2
- \frac{5 \pi}{72} \frac1{G_0 \Lambda_0} \right] G_0^2 k^4 + \mathcal{O}(k^6) \right],
\\
\Lambda(k) &=& \Lambda_0 \left[ 1 - \frac{9 + \pi^2}{9 \pi} G_0 k^2
+ \left( \frac{9 + \pi^2}{9 \pi} \right)^2 G_0^2 k^4 + \mathcal{O}(k^6) \right]
+ G_0 k^4 \left[ - \frac{5 \pi^2 + 144 \zeta(3)}{72 \pi} + \mathcal{O}(k^2) \right].
\end{eqnarray}
