...
\begin{abstract}
The traditional interpretation of Intravascular Ultrasound (IVUS) images during percutaneous coronary intervention (PCI) is a time-intensive process that often lacks consistency, as it heavily depends on the expertise and subjective judgment of physicians. Moreover, the collection and integration of IVUS data across multiple hospital systems are significantly hindered by regulatory restrictions and privacy concerns, creating barriers to collaborative analysis and model development. To develop a consistent and efficient method for IVUS image segmentation and improve plaque detection in PCI procedures, we developed a parallel 2D U-Net model with a multi-stage segmentation architecture. The model utilizes a federated learning algorithm to enable collaborative data analysis across multiple healthcare institutions. The model efficiently segments plaques by identifying and subtracting the external elastic membrane (EEM) and lumen areas. To enhance processing efficiency, polar coordinates are generated from Cartesian coordinates during the preprocessing stage. This approach ensures precise segmentation and optimizes computational performance in distributed medical imaging workflows. The model effectively identifies plaque positions with a Dice Similarity Coefficient (DSC) of 0.706 and accurately detects circular boundaries in real-time. In collaboration with domain experts, the IVUS segmentation system enhances plaque burden interpretation through precise quantitative parameter measurement. There is potential to further improve performance by integrating advanced federated learning methods. The technology is flexible and can be used anywhere sensitive and distributed data is present. Including more clients and more data may also help hospitals achieve successful outcomes.
\end{abstract}
